...
infrastructure — three small, focused libraries that each replace a
$50–150/mo SaaS for a solo founder's content pipeline.
...
|---|---|---|
| Hetzner CX22 VPS | €4.50 | Ubuntu, 4 GB RAM, runs everything |
...
## Why one repo, three projects

These three libraries grew out of the same content infrastructure and
share an aesthetic: small, single-file, replace-a-SaaS, no async magic
when sync works. They're packaged in one repo for one URL to remember
and one issue tracker. Each has its own README, requirements.txt, and
LICENSE; pick whichever is useful and ignore the rest.
